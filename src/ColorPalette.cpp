...
ColorPalette::ColorPalette(const raylib::Vector2& startPos, const raylib::Vector2& size){
	this->startPos = startPos;
	this->size = size;
	this->rectangle = raylib::Rectangle(startPos, (Vector2){size.x, size.y * 6/5});
	this->value = 1.0f;
...
void ColorPalette::drawColorRect(){
	rectangle.Draw(raylib::BLACK);
}
raylib::Rectangle ColorPalette::getSquare(){
	return rectangle;
}
raylib::Color ColorPalette::getColorFromPos(const Vector2& position){
	float posy = position.y - 10;
	return ColorFromHSV(posy, saturation, value);
}
void ColorPalette::setSaturation(const float& saturation){
	if(saturation >= 0 && saturation <= 1){
		this->saturation = saturation;
	}
}
void ColorPalette::setValue(const float& value){
	if(value <= 1 && value >= 0){
		this->value = value;
	}
}
float ColorPalette::getValue(){
	return this->value;
}
float ColorPalette::getSaturation(){
	return this->saturation;
}
#endif
